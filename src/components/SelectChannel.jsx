...
export const SelectChannel = ({
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div
          className="absolute left-0 top-0 w-full rounded-15 pointer-events-none -z-10"
          style={{
      ...
      <button
        className={`flex-1 justify-between duration-300 flex w-full items-center text-15 gap-[10px] rounded-15 h-[55px] pr-[15px] bg-carbon pl-[15px] relative z-10 ${
      ...
      <ul
        ref={dropdownRef}
        className={`absolute left-0 top-full duration-300 w-full bg-carbon p-[15px] rounded-b-15 pb-[15px] pt-1 -mt-[6px] z-10 ${
          isOpen
      ...
      >
        {options.map((option, idx) => (
          <li
            key={idx}
            className={`text-[13px] leading-[200%] py-[11px] last:border-none last:pb-0 border-b border-[#303030] ${
              option === selectedOption ? "text-primary" : "text-grayCustom"
            } hover:cursor-pointer`}
            onClick={() => handleOptionClick(option)}
          >
            <div className="radio-container">
              <input type="radio" name="radio" id={idx} />
              <label className="!pl-8 flex h-[18px]" htmlFor={idx}>
                <span className="-mt-1 inline-block text-15">{option}</span>
              </label>
            </div>
          </li>
        ))}
